...
<div class="wrapper">
 ...
 <?php include'Header.php';
    ...
    $sql11= "SELECT institution_type,SUM(approved) AS approved,SUM(total) AS filled FROM quarterly_national_jobs n";
    ...
    <section class="content">
      <?php include("menu5.php"); ?>
		...
		<?php		
			...
			$sql11=$sql11." AND month='".$month."' AND year=$year GROUP BY institution_type ORDER BY institution_type";
                         
            		$display = $month.'  '.$year.'  '.$district_name.' '.$institution_type.' '.$facility_name.' '.$facility_type_name.' '.$ownership.' '.$job_category.' '.$job_classification.' '.$region_name." Institution Type";         
		...
		$sql11= "SELECT institution_type,SUM(approved) AS approved,SUM(total) AS filled FROM quarterly_national_jobs";

		$sql11=$sql11." WHERE month='".$month."' AND year=$year  GROUP BY institution_type ORDER BY institution_type";
              

                $display = $month.'  '.$year.'  '." By Institution Type";
		       }
		  ...
		  <th>% Vacant in Position</th>
			...
			$sql12= "SELECT SUM(total) AS filled_structure,SUM(male) AS male_structure,SUM(female) AS female_structure  FROM national_jobs n  WHERE institution_type='$institution_type' AND approved > 0";
			

			$sql13= "SELECT SUM(total) AS filled_not_on_structure  FROM national_jobs n  WHERE institution_type='$institution_type' AND approved = 0";
