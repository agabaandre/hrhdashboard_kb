...
<div class="wrapper">
 ...
 <?php include'Header.php';
    ...
    $sql11= "SELECT SUM(male) AS male,SUM(female) AS female,job_category,SUM(approved) AS approved,SUM(total) AS filled FROM national_jobs n";
    ...
    <section class="content">
      <?php include("menu2.php"); ?>
		...
		<?php		
			...
			$sql11=$sql11." GROUP BY job_category ORDER BY job_category";
                         
            		$display = $district_name.' '.$institution_type.' '.$facility_name.' '.$facility_type_name.' '.$ownership.' '.$job_category.' '.$job_classification.' '.$region_name." Job Category";         
			  
                      
		       }else{

				$sql11= "SELECT SUM(male) AS male,SUM(female) AS female,job_category,SUM(approved) AS approved,SUM(total) AS filled FROM national_jobs  GROUP BY job_category ORDER BY job_category";

			  
                $display = " By Job Category";
		       }
