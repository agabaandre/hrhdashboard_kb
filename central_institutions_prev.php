...
//$sql12= "SELECT DISTINCT(n.facility_name),SUM(approved) AS approved,SUM(total) AS filled FROM national_jobs n WHERE facility_type_name IN ('Regional Referral Hospital','Ministry','Specialised National Facility','National Referral Hospital')  ";
      ...
      <div class="box">
		...
		<?php		
			...
			$sql14=$sql14." AND month='".$month."' AND year=$year GROUP BY n.facility_name ORDER BY n.facility_name";
            		...
            		$display = $month.'  '.$year.'  '.$district_name.' '.$institution_type.' '.$facility_name.' '.$facility_type_name.' '.$ownership.' '.$job_category.' '.$job_classification.' '.$region_name." Central Institutions";  
			...
			$sql14= "SELECT DISTINCT(n.facility_name),SUM(approved) AS approved,SUM(total) AS filled  FROM quarterly_national_jobs n WHERE facility_type_name IN ('Regional Referral Hospital','Ministry','Specialised National Facility','National Referral Hospital')  ";
                ...
                $display = $month.'  '.$year.'  '." Central Institutions";
		       }
		  ...
		  <th>% Vacant in Position</th>
			...
			$sql12= "SELECT SUM(total) AS filled_structure,SUM(male) AS male_structure,SUM(female) AS female_structure  FROM national_jobs n  WHERE facility_name='$facility_name' AND approved > 0";
			

			$sql13= "SELECT SUM(total) AS filled_not_on_structure  FROM national_jobs n  WHERE facility_name='$facility_name' AND approved = 0";
