...
<div class="wrapper">
 ...
 $sql11= "SELECT COUNT(DISTINCT(facility_name)) AS units,n.facility_type_name,n.district_name,SUM(approved) AS approved,SUM(total) AS filled  FROM national_jobs n WHERE  n.facility_type_name='City Health Office'";


    $display = 'City Health Offices';
    ...
    <section class="content">
      ...
      <div class="box">
		...
		<?php		
			...
			$sql11=$sql11." AND month='".$month."' AND year=$year GROUP BY n.district_name ORDER BY n.district_name";
                         
            		$display = $month.'  '.$year.'  '.$district_name.' '.$institution_type.' '.$facility_name.' '.$facility_type_name.' '.$ownership.' '.$job_category.' '.$job_classification.' '.$region_name." City Health Office"; 
              ...
              $sql11= "SELECT COUNT(DISTINCT(facility_name)) AS units,n.facility_type_name,n.district_name,SUM(approved) AS approved,SUM(total) AS filled  FROM quarterly_national_jobs n WHERE  n.facility_type_name='City Health Office'  ";
		...
		$sql11=$sql11." AND month='".$month."' AND year=$year  GROUP BY n.district_name ORDER BY n.district_name";
                ...
                $display = $month.'  '.$year.'  '." City Health Office";
		       }
		  ...
		  <th>% Vacant in Position</th>
			...
			$sum_female_structure = 0;
						    ...
						    $facility_type_name = 'City Health Office';
			
			$sql12= "SELECT SUM(total) AS filled_structure,SUM(male) AS male_structure,SUM(female) AS female_structure  FROM national_jobs n  WHERE district_name='$district_name' AND facility_type_name='$facility_type_name' AND approved > 0";
			

			$sql13= "SELECT SUM(total) AS filled_not_on_structure  FROM national_jobs n  WHERE district_name='$district_name' AND facility_type_name='$facility_type_name' AND approved = 0";
